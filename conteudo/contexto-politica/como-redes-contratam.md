---
title: "Como as Redes Públicas Estão Contratando"
subtitle: "Diagnóstico do cenário atual: a IA embutida, a contratação invisível e o desafio da opacidade funcional"
author: "Aliança de IA para a Educação"
date: "2026"
persona: contexto-politica
tipo: contexto
---

# Como as Redes Públicas Estão Contratando

> Levantamento realizado no Portal Nacional de Contratações Públicas (PNCP) entre 2024 e fevereiro de 2026 identificou apenas 15 processos envolvendo soluções com funcionalidades de IA: e em **nenhum** dos contratos analisados foram encontradas cláusulas específicas referentes à IA.

---

## O Fenômeno da IA Embutida

A adoção de soluções de IA pelas redes públicas de ensino tem sido realizada de forma predominantemente **inconsciente**. Plataformas digitais incorporam algoritmos de aprendizado de máquina ou de processamento de linguagem natural em suas funcionalidades sem que tais mecanismos sejam explicitados nos contratos, nos manuais ou nas formações destinadas aos usuários.

O resultado é a **contratação invisível**: soluções de IA chegam às redes como "plataformas inteligentes", "avaliação adaptativa" ou "personalização", sem que a presença da IA seja declarada pelo fornecedor ou reconhecida pelo gestor. Quando mencionada, a IA aparece mais como atributo comercial do que como critério de contratação.

> "A IA chega às redes públicas como 'plataforma inteligente' ou 'avaliação adaptativa', sem que sua presença seja declarada pelo fornecedor ou reconhecida pelo gestor."

---

## O Que Diz o Levantamento do PNCP

O levantamento realizado no Portal Nacional de Contratações Públicas (PNCP) para o período de 2024 a fevereiro de 2026 revelou:

| Indicador | Resultado |
|-----------|-----------|
| Processos com soluções de IA identificados | 15 |
| Percentual por inexigibilidade | 80% |
| Contratos com cláusulas específicas de IA | 0 |
| Contratos com exigências de explicabilidade | 0 |
| Contratos com salvaguardas de privacidade | 0 |
| Contratos com indicadores de desempenho algorítmico | 0 |

Redes de todas as cinco regiões do país foram identificadas no levantamento, demonstrando que o fenômeno é nacional: e não restrito a um perfil específico de município.

---

## Opacidade Funcional

O cenário descrito é conhecido como **opacidade funcional**: professores e gestores interagem com sistemas que aparentam ser softwares comuns, mas na prática tomam decisões com base em inferências estatísticas sobre trajetórias escolares: notas, diagnósticos, recomendações de conteúdo: sem que ninguém saiba exatamente como.

**Problemas da contratação embutida:**
- **Opacidade funcional:** uso desinformado e sem consciência crítica
- **Responsabilidade difusa:** falta de clareza quanto à responsabilização por erros ou resultados injustos
- **Tratamento como software comum:** termos de referência genéricos que ignoram parâmetros de explicabilidade e segurança jurídica

> "Sem o reconhecimento formal da natureza da IA contratada, sem métricas de desempenho algorítmico e sem garantias de explicabilidade e privacidade, o uso responsável da IA fica comprometido e a responsabilidade por seus resultados torna-se difusa."

---

## Três Padrões de Contratação Observados

Apesar do cenário geral de opacidade, três padrões de contratação já estão em curso:

### 1. Contratação por Inexigibilidade

Utilizada quando a competição é considerada inviável (produto ou serviço exclusivo). Prevista no art. 74 da Lei nº 14.133/2021.

**Caso de referência: Joinville (SC):** A Secretaria Municipal de Educação contratou o Programa Letrus (leitura e escrita com IA) por inexigibilidade, em dezembro de 2024. O contrato inclui análise automatizada de produções textuais e relatórios pedagógicos. Foram definidos indicadores mínimos de serviço: disponibilidade da plataforma (≥95%) e resolução de dúvidas (≥90%).

**Ponto de atenção:** Inexigibilidade exige justificativa rigorosa de exclusividade. É a modalidade mais usada (80% dos casos), mas nem sempre a mais adequada.

---

### 2. Contratação por Licitação (Pregão)

Utilizada para bens e serviços comuns, cujas especificações podem ser objetivamente definidas no edital. Julgamento pelo menor preço.

**Caso de referência: Rio Verde (GO):** A Secretaria Municipal contratou, por pregão eletrônico, sistema de correção digital de gabaritos para apoiar o SAERV (Sistema de Avaliação Educacional de Rio Verde), com leitura automática de cartões de resposta, geração de resultados em tempo real e relatórios de desempenho. O contrato de 12 meses foi posteriormente prorrogado por igual período.

**Ponto de atenção:** O pregão exige que o objeto seja bem especificado. Para IA, isso significa incluir critérios técnicos específicos no Termo de Referência.

---

### 3. Parceria Intermediada por Instituições

Municípios contratam com instituições parceiras (institutos, fundações, Sebrae) que realizam seleção prévia de soluções e atuam como executores institucionais.

**Caso de referência: Itajá (RN) via Sebrae/RN:** O Sebrae/RN realizou seleção de startups no âmbito do IA Lab Sebrae. O município de Itajá contratou o Sebrae/RN para implementar a solução IDES (Índice de Desenvolvimento Sebrae), que utiliza IA para planos de ação pedagógicos baseados em avaliações diagnósticas. Fundamento jurídico: art. 75, inciso XV, da Lei nº 14.133/2021 (dispensa de licitação para instituições sem fins lucrativos voltadas à inovação).

**Ponto de atenção:** O modelo intermediado pode simplificar o processo, mas a responsabilidade pedagógica e jurídica continua sendo da rede pública.

---

## O Caso da Seduc de Recife: Desenvolvimento Interno

A Secretaria de Educação do Recife adotou caminho diferente: **desenvolveu internamente** sua solução de IA, priorizando soberania sobre os dados, adequação ao contexto local e preservação da autoria docente.

As ferramentas incluem correção automatizada de produções estudantis, assistente pedagógico generativo para elaboração de questões e planos de aula alinhados à BNCC, e o aplicativo Revisa (leitura e estudo). O desenvolvimento é gerido pela própria Seduc com validação pedagógica prévia à etapa técnica.

---

## Para Onde Ir?

O diagnóstico é claro: a IA já está nas redes, mas a governança ainda não chegou. O caminho é transitar da **adoção invisível** para uma **adoção consciente, governada e pedagogicamente intencional**.

Este é o propósito do *Guia Prático de Contratação de IA na Educação*: oferecer aos gestores os instrumentos para navegar nesse processo com segurança.

---

*Aliança de IA para a Educação | Instituto Jataí | 2026*
